...
*/
/*int prefixe(int noeudA, int noeudB)
{
	int bidule[MAX_NB_NOEUDS];
	for(int i = 0; i < MAX_NB_NOEUDS; i++)
	    bidule[i]=0;
	while(noeudA != -1)
	{
	    bidule[noeudA]=1;
...
int dfs(Graph* graph, int source)/*, int depart, int flag, int pere)*/
{
	...
	while(!stack.empty())
	{
	    int noeud = stack.back();
	    stack.pop_back();
	for(int j = 0; j < graph->arcs[noeud].size(); j++)//size(&graph->listeAdj[noeud]); j++)
	{
		//int nextArete = get(&graph->listeAdj[noeud], j);
		int next = graph->arcs[noeud][j];//getAutreNoeud(graph->aretes[nextArete], noeud);
		...
		if(next==source || (match[next]!=-1 && peres[match[next]]!=-1)) //next est vvu et il est matché (et pas à nous normalement).
		{
			...
			int cur = noeud;
			int prec = next;
		    vector<int> blossom(graph->nbNoeuds, 0);
